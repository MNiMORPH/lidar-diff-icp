#!/usr/bin/env python3
"""Forest (PyForestScan cover>=0.5): PDF of near-nadir ground-return elevations vs PDF of ALL ground-return
...
d=Z["d_mm"]; sa=Z["scan_angle"]; slp=Z["slope"]; cf=Z["pfs_forest"]; ing=Z["in_grid"]
...
fig.suptitle(f"Forest (PyForestScan cover>=0.5): near-nadir vs all ground-return elevation PDFs, by slope{TITLE_TAG}",y=1.02)
fig.savefig(f"figures/refdatum/nadir_vs_all_pdf_by_slope{TAG}.png",dpi=100,bbox_inches="tight"); plt.close(fig)

print("forest (PFS cover>=0.5), ground-return elevation medians (mm):")
...
print(f"\nwrote figures/refdatum/nadir_vs_all_pdf_by_slope{TAG}.png")
