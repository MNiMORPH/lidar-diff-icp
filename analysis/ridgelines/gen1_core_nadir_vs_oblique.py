#!/usr/bin/env python3
"""CORE forest, gen1 ground returns: relative elevation vs intensity, split by scan geometry —
NEAR-NADIR (|angle|<=2 deg) vs OBLIQUE (|angle|>=8 deg). Do the intensity->elevation (range-walk)
law and the overall elevation level differ between nadir and oblique pulses?

    env -u PROJ_DATA -u GDAL_DATA ./lidar-icp/bin/python analysis/ridgelines/gen1_core_nadir_vs_oblique.py
"""
...
for a,(d,I,mx,my,lbl) in zip(ax,[(dn,iN,mxn,myn,"NEAR-NADIR (|angle|<=2deg)"),(do,iO,mxo,myo,"OBLIQUE (|angle|>=8deg)")]):
    hb=a.hexbin(I,d,gridsize=45,bins="log",cmap="viridis",mincnt=1,extent=(0,60,-400,300))
    a.plot(mx,my,"r.-",lw=2.5,ms=10,label="median d")
    a.set_xlim(0,60); a.set_ylim(-350,250); a.set_xlabel("ground-return intensity")
...
fig.suptitle("gen1 CORE forest ground: elevation vs intensity — near-nadir vs oblique pulses",y=1.0)
fig.savefig("figures/refdatum/gen1_core_nadir_vs_oblique.png",dpi=130,bbox_inches="tight"); plt.close(fig)
print("\nwrote figures/refdatum/gen1_core_nadir_vs_oblique.png")
