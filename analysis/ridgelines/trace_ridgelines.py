...
furrows in the fields). QC each threshold by how much of the divide set sits on topographic
highs.

Generic over tiles: grid (origin, res) is read from the tile's corrections*/meta JSON, and
the DEM is loaded from the tile dir. A tile with just z_after + a corrections json traces.

The field-furrow and forest QC percentages that used to appear here were derived from
`penetration.npy`, RETIRED 2026-09-05 -- it was geometry-confounded (ground-return fraction
...
def trace_ridgelines(z, res, *, threshold=200, smooth_sigma_m=12.5, tpi_window_m=305.0,
    ...
    convexity/analysis downstream is measured on the ORIGINAL DEM.
    """
    zf = _fill_nan(z)
...
def main():
    ...
    ridge, qc, sweep = trace_ridgelines(z, res, threshold=a.threshold)

    print(f"{'threshold':>10} {'cells':>8} {'%highs':>7}")
    for T, s in sorted(sweep.items()):
        print(f"{T:>10} {s.get('cells', 0):>8} "
              f"{s.get('pct_on_highs', float('nan')):>6.0f}%")
