...
| 6 | `analysis/modules/vegetation_correction/q2_cover_fit.py` | 52 | `ok = stable & (n1[cells] >= 5) & (ng >= 10) & …` | **buried, hardcoded** | 1,302 of 92,404 stable cells (1.41%) — but **cover-enriched**: median cover 0.346 vs 0.185 kept; top cover bin loses 33 of 102 cells (32%) | **MED — principle; the committed slope survives** |
...
| 11 | `analysis/tools/cover_offset_reference.py` | 48 | `--min-n` default `200` (returns per cover bin) | CLI | **nothing** — sparsest bin is n=273 (elba divides+inc5) / n=400 (elbaext) | harmless today, **latent** |
