...
> **CORRECTION 2026-09-01 -- the metric is "fraction above 0.14 m", not 0.15.** `lowveg`
> is read off STORED 20 mm histograms with a bin-CENTRE test, so the (0.14, 0.16] bin is
> counted whole. Verified on mark `1030_2022_MN` by recomputing from its own box cloud:
> identical histograms (7960 vs 7957 returns, per-bin agreement within LAS quantisation)
> but numerator **520 exact against 705 binned**. The coefficient below is correct FOR THE
> BINNED METRIC and must not be applied to an exactly-computed one. The rank correlation
> is unaffected; the scale is not.
>
> Recomputed exactly from the box clouds, the same relation refits to
> `-325.2 +/- 35.4 mm per unit` with a free intercept of `-8.7 +/- 4.2` (block bootstrap
