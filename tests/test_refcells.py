...
def test_all_stable_keeps_everything(tile):
    m, rep = reference_cells(tile, exclude_valley=False)
...
def test_gross_change_cannot_remove_a_vegetation_scale_offset(tile):
    ...
    m, _ = reference_cells(tile, exclude_valley=False)
...
def test_cells_subset_and_report_order(tile):
    ...
    m, rep = reference_cells(tile, cells=np.array([4, 5, 6]), exclude_valley=False)
...
def test_the_valley_cut_needs_a_STATED_elevation_and_refuses_to_guess(tile):
    """Andy, 2026-09-04: the floodplain is cut at a stated elevation. Not TPI -- at
    whitewater that mask misses 150,873 cells of valley floor. And not the histogram
    antimode either: that worked at elba only because elba is genuinely bimodal, and at
    whitewater it returned an arbitrary point in a decaying tail and cut 76% of the tile.
    A site with no established value must RAISE, because a wrong valley top silently
    changes which ground counts as no-change."""
    import numpy as np
    np.save(tile / "z_after.npy", np.full((4, 4), 300.0))
    with pytest.raises(ValueError, match="valley_top_m"):
        reference_cells(tile)
    m, rep = reference_cells(tile, valley_top_m=310.0)
    assert any("below valley top 310.0 m (stated)" in k for k in rep), rep
    assert m.sum() == 0                      # everything is below 310 m -> all cut


def test_established_valley_tops_come_from_the_registry():
    from lidar_diff_icp.refcells import VALLEY_TOP_M
    assert VALLEY_TOP_M["elba"] == 230.0     # run_steady_state_strata.py VALLEY_TOP
