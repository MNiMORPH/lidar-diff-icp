...
def register_gen1(before_laz, bounds, res, *, ground_source="csf", csf_pdal=None,
    ...
    # reference line's own error. Measured on elbaext with the SHIPPED tie ("intercept"): the six per-swath dz
    # span 44.60 mm (-22.60 .. +22.00), so re-gauging on a different line moves every
    # elevation by up to
...
def difference_dem(before_laz, after_laz, bounds, *, res=5.0, ground_q=0.50,
    ...
    **The absolute level of the output depends on the ZERO LINE and is not a measured
    ...
    elevation (measured span 44.60 mm at elbaext, tie="intercept"). Swath-to-swath DIFFERENCES are
    ...
    _grid = (X0, Y0, res, nx, ny)

    def cellstat(x, y, z, how, q=None):
        return groundest.cellstat(x, y, z, how, _grid,
    ...
    def groundg(x, y, z):
        """The chosen estimator, from groundest. slope_normal needs the regional plane."""
        return groundest.estimate_ground(
            x, y, z, _grid, _GQ_SCALAR, ground,
            plane=((Zreg_f, dzde, dzdn) if ground == "slope_normal" else None))
